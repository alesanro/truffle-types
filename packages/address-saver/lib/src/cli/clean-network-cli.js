...
const artifact_saver_1 = require("../artifact-saver");
const args = yargs
    .option("addresses", {
    type: "string",
    demandOption: true,
    ...
    .argv;
if (args.network) {
    artifact_saver_1.removeDeployedNetworkSync(args.network, args.addresses);
    console.info(`Network ${args.network} was removed.`);
}
else if (args.saveNetworks && args.saveNetworks.length > 1) {
    artifact_saver_1.removeDeployedNetworkExceptSync(args.saveNetworks, args.addresses);
    console.info(`All networks except ${args.saveNetworks} were removed.`);
}
else if (args.removeAll) {
    artifact_saver_1.removeDeployedNetworkExceptSync([], args.addresses);
