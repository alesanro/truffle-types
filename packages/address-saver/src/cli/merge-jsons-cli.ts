...
const args = yargs
  ...
  .option("target", {
    ...
    demand: true,
    describe:
      "Target of the merge. If no '--output' will be provided then results will saved to target path"
  })
  .option("output", {
    type: "string",
    demand: false,
...
const output = resolve(args.output ? args.output : args.target);
const merged = deepmerge(inputs.source, inputs.target);

writeFileSync(output, JSON.stringify(merged, undefined, "\t"), {
  encoding: "utf8"
});
