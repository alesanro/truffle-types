...
export default class ContractDeploymentContext {
    ...
    public async saveDeployedContractsAsync(addresses: ArtifactRecord[], networkId?: number): Promise<void> {
        if (!this.skipLogs) {
            for (const deployedContract of addresses) {
                this.logger.info(
                    `${deployedContract.name} [${deployedContract.contract}] deployed at ${deployedContract.address}`,
                );
            }
        }
    ...
    public async getUnwrappedContractInstanceAsync<T>(
        ...
        const contractObj = await this.getUnwrappedDeployedContractAsync(name, networkId);

        if (contract.contractName !== contractObj.contract) {
            console.warn(
                `[deployment-context] Possibly wrong contract passed for address ${contractObj.address}. Got ${
                    contract.contractName
                }, expected ${contractObj.contract}`,
            );
        }
