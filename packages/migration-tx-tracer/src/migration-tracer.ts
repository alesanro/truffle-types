...
import {
    ...
    EmptyLoggerProvider,
    TransactionLogger,
    TransactionDestinationSubprovider,
    TransactionDestinationEvents,
    TxDestinationEventCallback
} from "@truffle-types/web3-subproviders";
...
export class MigrationSetupConfigurator {
    ...
    private _migrationCompletedStepListener: TxDestinationEventCallback = (
        toAddress,
        sig
    ) => {
        if (
            !(
                this.migrationsAddress &&
                toAddress === this.migrationsAddress &&
                sig === MIGRATIONS_SET_COMPELETED_SIG
            )
        ) {
            return;
        }
    ...
    setupMigrationHooks() {
        MigrationController.beforeMigrationStaticHook = async (
            ...
            this.destinationSubprovider.emitter.removeListener(
                TransactionDestinationEvents.TxDestination,
                this._migrationCompletedStepListener
            );
            ...
            this.currentLogProvider = new FileLoggerProvider(
                join(
                    parsedPath.dir,
                    `${network}_txlogs`,
        ...
        MigrationController.afterMigrationStaticHook = async (
            ...
            if (MigrationsContract.isDeployed()) {
                ...
                this.destinationSubprovider.emitter.on(
                    TransactionDestinationEvents.TxDestination,
                    this._migrationCompletedStepListener
                );
            }
