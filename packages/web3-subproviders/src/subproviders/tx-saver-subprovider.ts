    ...
    private _sendTransaction(
        ...
        if (self.txLogger.checkLogExists(txHashsum)) {
            ...
            if (foundLogRecord.status === "pending") {
                ...
                return end(
                    new Error(
                        `[MigrationSaver:handleRequest]: Unsupported log status "${foundLogRecord.status}" in tx ${
            ...
            self.web3.eth.getTransaction(record.txhash, (err: Error, transaction: Web3.Transaction) => {
                ...
                if (!transaction) {
                    ...
                    return waitForNodeResponse(
                        record.txhash,
                        self.txPollMs,
                        resolveAsTxComplete(record.txhash, (err, result) => end(err, result)),
                    );
        ...
        function resolveAsTxComplete(
            ...
            return (err, receipt, status): void => {
                if (!receipt && !status) {
                    ...
                    // tslint:disable-next-line:no-null-keyword
                    return callback(
                        new Error(`[MigrationSaver:resolveAsTxComplete]: Undefined error occured ${err}`),
                        null,
                    );
                }
            };
